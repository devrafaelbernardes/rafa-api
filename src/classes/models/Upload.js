...
export const Upload = () => {
  ...
  const defineFilename = (name) =>
    `file${Date.now()}-${String(name).replace(/[^A-Za-z0-9.]+/g, "")}`;
  ...
  return {
    ...
    uploadVideo: (file, name, description) =>
      upload({ file, name, description, typeUpload: TYPES_UPLOAD.VIDEO }),
    uploadMaterial: (file) =>
      upload({ file, typeUpload: TYPES_UPLOAD.MATERIAL }),
    uploadModeling: (file) =>
      upload({ file, typeUpload: TYPES_UPLOAD.MODELING }),
  };
};
